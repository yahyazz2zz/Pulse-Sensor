            ...
            if (N > 250){                                        // avoid high frequency noise
                if ((Signal > thresh) && (Pulse = false) && (N > (IBI / 5.0) * 3) ){
                    ...
                    if (secondBeat = true) {                     // if this is the second beat, if secondBeat == TRUE
                        ...
                        for (int i = 0; i < rate.length; i++){          // seed the running total to get a realisitic BPM at startup
                            rate[i] = IBI;
                        }
                    }
                    ...
                    int temp = rate.length;                     // shift data in the rate array
                    for (int i = (rate.length - 1); i >= 0; i--) {
                        rate[i+1] = rate[i];
                    }
                    rate[-1] = IBI;                             // add the latest IBI to the rate array
                    int sum = IntStream.of(rate).sum();
                    int runningTotal = sum;                     // add upp oldest IBI values
